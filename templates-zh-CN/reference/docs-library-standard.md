...
| 位置 | 职责 |
| --- | --- |
| `coding-agent-harness/governance/generated/Harness-Ledger.md` | 由任务本地事实生成的全局上下文维护投影。 |
| `coding-agent-harness/governance/lessons/` | promoted lesson 详情、治理记录、归档。 |
...
| `coding-agent-harness/governance/regression/` | Regression gates、Cadence rules、测试策略和回归证据索引。 |
| `coding-agent-harness/planning/` | Delivery Plan、Module Registry、任务目录、模块计划。 |
| `coding-agent-harness/planning/tasks/` | 每个任务一个目录，包含 task plan、progress、findings、review、walkthrough。 |
| `coding-agent-harness/governance/generated/Closeout-Index.md` | 从任务本地 walkthrough 生成的收口投影。 |
| `coding-agent-harness/governance/standards/` | agent 按需加载的标准文件，控制行为而不是记录流水账。 |
| `coding-agent-harness/tmp/` | 临时材料，定期清理，不作为长期事实来源。 |
...
| 收口记录 | `planning/tasks/<task>/walkthrough.md` | 给下一轮 agent 的可追溯交接。 |
| Closeout Index | `governance/generated/Closeout-Index.md` | 从 task-local walkthrough 生成的收口索引。 |
...
- 生成文件固定放在 `governance/generated/`，不要手工维护任务行。
- `coding-agent-harness/tmp/` 下超过 7 天未更新的文件应清理或迁移到正式位置。
...
5. 更新 reference 或 template 时，必须在任务本地记录本次上下文维护，并用生成器刷新 Harness Ledger。
