...
fs.writeFileSync(progressPath, originalProgress.replace(/^##\s+Current Status\s*\n\nplanned/im, "## 状态：计划审阅中"));
const invalidStateStatusResult = run(["status", "--json", target]);
assert(invalidStateStatusResult.stdout.trim().startsWith("{"), "invalid task state status should still emit JSON");
const invalidStateStatus = JSON.parse(invalidStateStatusResult.stdout) as HarnessTestLooseJson;
const invalidStateTask = invalidStateStatus.tasks.find((task) => task.id === taskId);
assert(invalidStateTask.state === "unknown", "invalid explicit progress state should preserve unknown semantic state");
assert(invalidStateTask.stateSource === "invalid", "invalid explicit progress state should preserve invalid stateSource");
assert(invalidStateTask.stateRaw === "计划审阅中", "invalid explicit progress state should preserve raw value");
const invalidStateCheckFailures = ((invalidStateStatus.checkState.details as Record<string, unknown>).failures as string[]) || [];
assert(invalidStateCheckFailures.some((failure) => failure.includes("invalid task state") && failure.includes("计划审阅中")), "status checkState should expose invalid raw task state");
assert(invalidStateTask.materialIssues.some((issue) => issue.code === "invalid-task-state"), "invalid explicit progress state should become a material issue");
assert(invalidStateTask.queueReasons.some((reason) => reason.code === "invalid-task-state"), "invalid explicit progress state should become a queue reason");
assert(invalidStateTask.taskQueues.includes("missing-materials"), "invalid explicit progress state should route to repairable missing-materials");
assert(!invalidStateTask.taskQueues.includes("unknown"), "invalid explicit progress state should not disappear into only the unknown queue");
assert(invalidStateTask.repairPrompt.includes("计划审阅中"), "repair prompt should include the raw invalid state");
assert(invalidStateTask.repairPrompt.includes("planned"), "repair prompt should include allowed task states");
assert(invalidStateTask.repairPrompt.includes("progress.md"), "repair prompt should point to progress.md");
const invalidStateIndexResult = run(["task-index", "--json", target]);
assert(invalidStateIndexResult.stdout.trim().startsWith("{"), "invalid task state index should still emit JSON");
const invalidStateIndex = JSON.parse(invalidStateIndexResult.stdout) as HarnessTestLooseJson;
const invalidStateIndexedTask = invalidStateIndex.tasks.find((task) => task.taskKey === taskId);
assert(invalidStateIndexedTask.materialIssues.some((issue) => issue.code === "invalid-task-state"), "task-index should expose invalid task state material issue");
assert(invalidStateIndexedTask.queueReasons.some((reason) => reason.code === "invalid-task-state"), "task-index should expose invalid task state queue reason");
assert(invalidStateIndexedTask.repairPrompt.includes("计划审阅中"), "task-index repair prompt should include the raw invalid state");
fs.writeFileSync(progressPath, originalProgress);
...
const deleteCollisionFixture = expectJson(["new-task", "queue-delete-collision", "--title", "Queue Delete Collision", "--locale", "en-US", target]);
const deleteCollisionDir = taskDirectory(deleteCollisionFixture);
const deleteCollisionPlan = path.join(deleteCollisionDir, "task_plan.md");
const deleteCollisionProgress = path.join(deleteCollisionDir, "progress.md");
const deleteCollisionPlanBefore = fs.readFileSync(deleteCollisionPlan, "utf8");
const deleteCollisionProgressBefore = fs.readFileSync(deleteCollisionProgress, "utf8");
const deleteCollisionArchiveDir = path.join(target, "coding-agent-harness/governance/archive/soft-deleted/tasks/TASKS", `${todayLocal}-queue-delete-collision`);
fs.mkdirSync(deleteCollisionArchiveDir, { recursive: true });
fs.writeFileSync(path.join(deleteCollisionArchiveDir, "existing.txt"), "existing archive collision\n");
const deleteCollision = run(["task-delete", "queue-delete-collision", "--reason", "collision fixture", target]);
assert(deleteCollision.status !== 0, "task-delete should reject archive destination collisions before mutating source files");
assert(`${deleteCollision.stdout}\n${deleteCollision.stderr}`.includes("Tombstone destination already exists"), "destination collision failure should explain the archive path conflict");
assert(fs.existsSync(deleteCollisionDir), "failed soft delete destination collision should keep the active task directory in place");
assert(fs.readFileSync(deleteCollisionPlan, "utf8") === deleteCollisionPlanBefore, "failed soft delete destination collision must not write a tombstone before moving");
assert(fs.readFileSync(deleteCollisionProgress, "utf8") === deleteCollisionProgressBefore, "failed soft delete destination collision must not append progress before moving");
fs.rmSync(deleteCollisionArchiveDir, { recursive: true, force: true });
