...
export function copyDashboardAssets(outDir: string, options: DashboardWriterOptions = {}): void {
  copyDirectory(dashboardTemplateRootForLocale(options.locale), outDir);
}

export function writeDashboardDirectory(outDir: string, bundle: AnyRecord, options: DashboardWriterOptions = {}): string {
...
export function writeDashboardFile(outFile: string, bundle: AnyRecord, options: DashboardWriterOptions = {}): string {
...
export function renderDashboardFile(bundle: AnyRecord, options: DashboardWriterOptions = {}): string {
  const templateRoot = dashboardTemplateRootForLocale(options.locale);
  const readAsset = (relativePath: string) => fs.readFileSync(path.join(templateRoot, relativePath), "utf8");
...
function renderDashboardIndex(locale = "en-US", options: DashboardWriterOptions = {}): string {
...
function readDashboardApp(templateRoot: string): string {
...
function assertSafeDashboardTarget(target: string, options: DashboardWriterOptions): void {
  ...
  for (const root of [options.docsRoot, dashboardTemplateRoot, localizedDashboardTemplateRoot].filter((item): item is string => Boolean(item)).map((item) => path.resolve(item))) {
...
function looksLikeGeneratedDashboardDirectory(target: string): boolean {
  const full = [
    "index.html",
    "README.md",
    "assets/app.js",
...
function dashboardTemplateRootForLocale(locale = "en-US"): string {
