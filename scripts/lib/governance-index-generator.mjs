...
function taskIndexSurfaces(target, tasks) {
  const planningGeneratedRoot = target.structureVersion === 2
    ? path.join(target.planningRoot, "generated")
    : path.join(target.planningRoot, "generated");
  ...
  if (target.structureVersion === 2) {
    ...
    surfaces.push({
      ...
      absolute: target.closeoutIndexPath,
      relative: toPosix(path.relative(target.projectRoot, target.closeoutIndexPath)),
      rows: closeoutRows,
  ...
  return [
    {
      ...
      absolute: target.ledgerPath,
      relative: toPosix(path.relative(target.projectRoot, target.ledgerPath)),
...
function legacyFeatureSurfaces(target) {
  return [
    ["legacy-feature-ssot", path.join(target.planningRoot, "Feature-SSoT.md")],
    ["legacy-private-feature-ssot", path.join(target.planningRoot, "Private-Feature-SSoT.md")],
...
function uniqueArchiveDir(target) {
  ...
  const base = target.structureVersion === 2
