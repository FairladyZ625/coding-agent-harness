...
graph TD
  ...
  DASH["dashboard<br/>只读可视化"]
  SA["safe-adoption<br/>旧项目安全接入"]
  ...
  style DASH fill:#59636e,color:#fff
  style SA fill:#59636e,color:#fff
```

简单项目只装 Core。复杂项目按需装能力包。`subagent-worker` 依赖 `module-parallel`。
v1.0 registry 使用这些 canonical names；旧草稿中的 `review-contract` 只作为
`adversarial-review` 的兼容别名。
...
flowchart TB
  ...
  Decide["2 Decide<br/>选语言 + delivery model + capability packs"]
...
`Scaffold` 必须尊重安装语言：`zh-CN` 生成中文操作模板，`en-US` 生成英文操作模板。
非交互场景使用 `harness init --locale zh-CN|en-US`，Agent 引导安装时先询问用户。
